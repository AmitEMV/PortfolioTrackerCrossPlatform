...
namespace PortfolioTracker.Services
{
    public class PortfolioService : IPortfolioService
    {
        ...
        public async Task<FundDistribution[]> GetFundDistributionAsync()
        {
            try
            {
                var response = await GetHttpClient.GetAsync(new Uri($"{apiURL}/api/portfolio/funddistribution"));
        ...
        public async Task<List<PortfolioSnapshot>> GetPortfolioSnapshotAsync()
        {
            try
            {
                var response = await GetHttpClient.GetAsync(new Uri($"{apiURL}/api/portfolio/portfoliosnapshot"));
                var content = await response.Content.ReadAsStringAsync();
                JsonSerializerOptions options = new JsonSerializerOptions()
                {
                    IgnoreNullValues = true,
        ...
        public async Task<InvestmentStatus> GetInvestmentReturnsValueAsync()
        {
            try
            {
                var response = await GetHttpClient.GetAsync(new Uri($"{apiURL}/api/portfolio/investmentvalue"));
                var content = await response.Content.ReadAsStringAsync();
                JsonSerializerOptions options = new JsonSerializerOptions()
                {
                    IgnoreNullValues = true,
        ...
        public async Task<bool> AddFundToPortfolioAsync(FundInfo fundInfo)
        {
            ...
            try
            {
                var response = await GetHttpClient.PostAsync(new Uri($"{apiURL}/api/portfolio/addfund"), new StringContent(JsonSerializer.Serialize(fundInfo), System.Text.Encoding.UTF8, "application/json"));
                if (response.IsSuccessStatusCode)
                {
                    var content = await response.Content.ReadAsStringAsync();
                    JsonSerializerOptions options = new JsonSerializerOptions()
